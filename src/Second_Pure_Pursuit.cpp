...
PurePursuit::PurePursuit()
{
    ...
    // Car parameter
    pravite_nh.param("base_shape_L", base_shape_L, 0.9); // 机器人轴距
    pravite_nh.param("reference_v", reference_v, 0.7);    // 目标速度
    pravite_nh.param("Lfw", Lfw, 0.3);                    // 前视距离
    ...
    odom_sub_ = nh_.subscribe("/odom_link", 1, &PurePursuit::odomCallback, this);
...
void PurePursuit::laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
    {
...
bool PurePursuit::findAvoidance(const sensor_msgs::LaserScan::ConstPtr& msg)
{   
...
geometry_msgs::Point PurePursuit::get_odom_car2WayPtVec(const geometry_msgs::Pose &carPose)
{
    ...
    if (!goal_reached_)
    {
        ...
        for (int i = index; i < map_path_.poses.size(); i++)
        {
            ...
            geometry_msgs::Point odom_path_wayPt = map_path_.poses[i].pose.position;
...
void PurePursuit::controlLoopCallback(const ros::TimerEvent &)
{
    ...
    // ackermann angle/speed to twist
    // this->twist_cmd_.angular.z = (this->velocity * tan(this->steering_)* 2.04) / -8.6;
    this->twist_cmd_.angular.z = ((this->velocity * 2 * tan(getSteering(eta)) ) / Lfw) / -8.6;
